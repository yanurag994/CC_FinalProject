...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mssql+pymssql://'+username+':'+password+'@'+server+':1433/'+database
    ...
    if request.method == "POST":
        ...
        with open(os.path.join(app.config['UPLOAD_FOLDER'], user_hash + '_in_' + file.filename), 'r') as f:
            ...
            for row in reader:
                ...
                cursor.execute("INSERT INTO household_data (HSHD_NUM, L, AGE_RANGE, MARITAL, INCOME_RANGE, HOMEOWNER, HSHD_COMPOSITION, HH_SIZE, CHILDREN) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]))
        ...
        with open(os.path.join(app.config['UPLOAD_FOLDER'], user_hash + '_in_' + file.filename), 'r') as f:
            ...
            for row in reader:
                row = [None if val.strip() == 'null' else val.strip() for val in row]
                cursor.execute("INSERT INTO product_data (PRODUCT_NUM,DEPARTMENT,COMMODITY,BRAND_TY,NATURAL_ORGANIC_FLAG) VALUES (%s, %s, %s, %s, %s)",(row[0], row[1], row[2], row[3], row[4]))
